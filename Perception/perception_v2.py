...
class PerceptionNode:
    ...
    def detected_symbol_callback(self, msg):
        ...
        # for detection in self.detected_symbols:
    ...
    def handle_path_tracking(self):
        label_detected = self.get_detected_label(["object_1", "object_2", "highwayentry", "highwayend", "redlight", "stop", "roundabout"], 500.0)
    ...
    def handle_tunnel(self):
        if self.is_detected_tunnel(angle_offsets=[90, 70, 50], wall_distance_thresh=0.9, mode="exit"):
            self.publish_mission_flag("path_traking")
            self.transition_to(1)

    def handle_stop_obstacle(self):
        ### 라이다 범위 확인 필요 ###
        if not self.is_detected_obstacle((-160,-180), (0, 20), (0.01, 0.05)):
    ...
    def handle_roundabout(self):
        if self.roundabout_init_flag:
            if self.is_detected_obstacle((-160,-180), (0, 0), (0.01, 0.05)):
                self.roundabout_init_flag = False
                self.publish_mission_flag("path_tracking")
            
        if self.is_detected_obstacle((-160,-180), (0, 20), (0.01, 0.05)):
            ### 라이다 범위 확인 필요 ###
            self.publish_mission_flag("stop_second")
            self.transition_to(5)
