...
######################################## 인식 파트 #######################################################
    ...
    def is_detected_tunnel(self, angle_offsets=[90, 110, 130], wall_distance_thresh=0.6, mode="entry"):
        ...
        def get_distance(angle_rad,mode):
            if mode == "entry":
        ...
        for la, ra in zip(left_angles, right_angles):
            dl = get_distance(la,mode)
            dr = get_distance(ra,mode)
...
######################################### 메인 동작 파트  ######################################################
    ...
    def handle_path_tracking(self):
        ...
        # Obstacle Detect
        if label_detected in ["object"] and self.is_detected_obstacle((160, 180), (-160, -180), (0.01, 1.0)):
...
######################################### 미션 동작 파트  ######################################################
    ...
    def handle_tunnel(self):
        if self.is_detected_tunnel(angle_offsets=[160, 135, 110], wall_distance_thresh=7.0, mode="exit"):
            rospy.loginfo("----------------------")
